...
Một số ký tự đặc biệt giống trên bảng mã [bảng mã ASCII](../../CS/common-ASCII.md) không phải một ký tự thuần như ký tự trên bàn phím. Các ký tự đó được gọi là các __*ký tự đặt biệt*__. Các ký tự ngoài khả năng biểu diễn một hình gì đó còn có tác dụng:
...
- Hàm __*printf*__ sẽ tự động chuyển đổi để phù hợp đầu ra. Ví dụ ở ký tự `'A'` nếu ép hiển thị dưới dạng số nguyên thập phân thì nó sẽ in ra giá trị của ký tự đó trên [bảng mã ASCII](../../CS/common-ASCII.md)
